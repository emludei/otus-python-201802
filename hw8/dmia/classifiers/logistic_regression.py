...
class LogisticRegression:
    ...
    def predict_proba(self, X, append_bias=False):
        ...
        predictions = 1.0 / (1.0 + np.exp(-X.dot(self.w.T)))
    ...
    def loss(self, X_batch, y_batch, reg):
        ...
        predictions = 1.0 / (1.0 + np.exp(-X_batch.dot(self.w.T)))
        loss = -np.sum((y_batch * np.log(predictions)) +
                       (1 - y_batch) * np.log(1 - predictions))

        dw = (predictions - y_batch).T * X_batch

        # Right now the loss is a sum over all training examples, but we want it
        # to be an average instead so we divide by num_train.
        # Note that the same thing must be done with gradient.

        trains = X_batch.shape[0]
        dw /= trains
        loss /= trains
        ...
        loss += (reg / (2 * trains)) * np.sum(self.w[:-1]**2)
        dw[:-1] += (reg * self.w[:-1]) / trains
